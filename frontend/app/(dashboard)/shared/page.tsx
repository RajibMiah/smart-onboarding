...
const SharedWithMePage = () => {
  const { items, isLoading, error } = useSharedFeed("with_me");

  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-5">
      <h1 className="text-2xl font-bold tracking-tight text-black">Shared with me</h1>
      ...
      {!isLoading && items.length === 0 ? (
        <div className="mx-auto mt-12 max-w-2xl rounded-2xl border border-dashed border-black/30 bg-white p-16 text-center">
          ...
          <h2 className="mt-3 text-lg font-bold text-black">Shared with me</h2>
          <p className="mt-1 text-sm text-neutral-500">Content colleagues share with you directly will show up here.</p>
        </div>
      ) : (
